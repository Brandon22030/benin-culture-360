...
const validateQuizQuestion = (question: Partial<QuizQuestion>): boolean => {
  ...
  if (typeof question.correctAnswer !== 'number') {
    ...
    console.warn(`Correction de l'index de réponse: ${question.correctAnswer} pour ${question.options?.length} options`);
    // Correction automatique de l'index de réponse
    question.correctAnswer = Math.min(question.correctAnswer, (question.options?.length || 1) - 1);
  }
  ...
  if (correctOption && !question.explanation?.toLowerCase().includes(correctOption.toLowerCase())) {
...
export const generateQuizQuestions = async (
  ...
  try {
    const result = await generateGeminiQuiz(numberOfQuestions, difficulty, 0, generationState);
    generationState.inProgress = false;
    return result;
  } catch (error) {
...
const generateGeminiQuiz = async (
  ...
  if (attempts >= 3 || 
      (generationState && generationState.currentAttempt >= generationState.maxAttempts)) {
    if (import.meta.env.DEV) {
      console.warn(`Nombre maximum de tentatives atteint. Utilisation des questions prédéfinies.`);
    }
    return fallbackQuestions(numberOfQuestions, difficulty);
  }
  ...
  if (import.meta.env.DEV) {
    console.log(`🔍 Génération de ${numberOfQuestions} questions (Difficulté: ${difficulty}, Tentative: ${attempts + 1})`);
  }
  ...
  try {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        console.error("❌ Erreur de parsing JSON :", parseError, "JSON brut :", jsonMatch[0]);
      }
      // Réessayer avec une nouvelle tentative
      return generateGeminiQuiz(numberOfQuestions, difficulty, attempts + 1);
    }
    ...
    const validQuestions = parsedQuizData.questions.filter(validateQuizQuestion);

    // Compléter avec des questions prédéfinies si moins de 10 questions valides
    if (validQuestions.length < numberOfQuestions) {
      console.warn(`Seulement ${validQuestions.length} questions valides générées. Complétion avec des questions de secours.`);
      const predefinedQuestions = fallbackQuestions(numberOfQuestions - validQuestions.length, difficulty);
      return [...validQuestions, ...predefinedQuestions].slice(0, numberOfQuestions);
    }
