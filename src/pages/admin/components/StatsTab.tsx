...
const StatsTab = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const [usersCount, galleryStats, musicStats, articlesCount, culturesCount] = await Promise.all([
      ...
      const pendingContributions = [...galleryData, ...musicData]
        .filter(item => item.status === 'pending').length;
      
      const approvedContributions = [...galleryData, ...musicData]
      ...
      setStats({
        ...
        approvalRate: totalContributions ? 
          Math.round((approvedContributions / totalContributions) * 100) : 0,
  ...
  return (
    ...
    {/* <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6"> */}
      <Card>
        ...
        <CardContent>
          <p className="text-3xl font-bold text-benin-green">{stats.totalUsers}</p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-3xl font-bold text-benin-green">{stats.totalContributions}</p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-3xl font-bold text-orange-600">{stats.totalCultures}</p>
        </CardContent>
      </Card>
    </div>
  );
